...
describe("OpenAPI Generation", () => {
  ...
  describe("OpenAPI Specification", () => {
    ...
    it("should mark AI-powered tools correctly", () => {
      const aiPoweredTools = Object.entries(openapiSpec.paths).filter(
        ([_, pathItem]: [string, any]) => pathItem.post["x-ai-powered"] === true
      );
  ...
  describe("Schema Consistency", () => {
    ...
    it("should have consistent tool names", () => {
      const pathToolNames = Object.keys(openapiSpec.paths).map((path) =>
        path.replace("/tools/", "")
      );
