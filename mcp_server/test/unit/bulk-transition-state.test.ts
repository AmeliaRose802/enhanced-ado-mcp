...
describe("Bulk Transition State Handler", () => {
  ...
  describe("State transition validation", () => {
    ...
    it("should detect invalid state transitions", async () => {
      ...
      const workItemContext = new Map([[101, { title: "Bug 1", state: "Removed", type: "Bug" }]]);
      ...
      expect(result.success).toBe(true);
      expect(result.warnings).toEqual(expect.arrayContaining([expect.stringContaining("Removed")]));
    });
  ...
  describe("Optional parameters", () => {
    it("should include reason when provided", async () => {
      ...
      const workItemIds = [101];
      const workItemContext = new Map([[101, { title: "Bug 1", state: "Active", type: "Bug" }]]);
