...
describe('Bulk Move to Iteration Handler', () => {
  ...
  describe('Iteration path validation', () => {
    it('should validate iteration path exists', async () => {
      ...
      const workItemContext = new Map([
        [101, { title: 'Task 1', state: 'Active', type: 'Task', iterationPath: 'TestProject\\Sprint 10' }],
        [102, { title: 'Task 2', state: 'Active', type: 'Task', iterationPath: 'TestProject\\Sprint 10' }]
      ]);
  ...
  describe('Item selection', () => {
    it('should move only selected items by index', async () => {
      ...
      const workItemContext = new Map(
        workItemIds.map(id => [
          id,
          { title: `Task ${id}`, state: 'Active', type: 'Task', iterationPath: 'TestProject\\Sprint 10' }
        ])
      );
  ...
  describe('Dry run mode', () => {
    ...
    it('should show current iteration path in preview', async () => {
      ...
      const workItemContext = new Map([
        [101, { title: 'Task 1', state: 'Active', type: 'Task', iterationPath: 'TestProject\\Sprint 10' }],
        [102, { title: 'Task 2', state: 'Active', type: 'Task', iterationPath: 'TestProject\\Backlog' }]
      ]);
  ...
  describe('Execution mode', () => {
    ...
    it('should handle partial failures', async () => {
      ...
      const mockPatch = jest.fn()
        .mockResolvedValueOnce({ data: { id: 101, fields: { 'System.IterationPath': 'TestProject\\Sprint 11' } } })
