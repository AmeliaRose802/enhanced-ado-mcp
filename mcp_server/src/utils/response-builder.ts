...
export function buildErrorResponse(
  ...
  const errorMetadata: ErrorMetadata = createErrorMetadata(errorCategory, code, metadata);
 ...
 */
export function buildAzureCliErrorResponse(error: {
  isAvailable: boolean;
  isLoggedIn: boolean;
  error?: string;
}): ToolExecutionResult {
  ...
  return buildErrorResponse(errorMsg, { source: "azure-cli-validation" }, category, code);
}
