...
export function detectHallucinatedIds(
  ...
  if (context?.recentIds && context.recentIds.length > 0) {
    ...
    if (providedMax < recentMin - 1000) {
      suspicionScore += 0.7;
      reasons.push(
        `Provided IDs (max: ${providedMax}) are far below recently seen IDs (min: ${recentMin})`
      );
    }
  }
  ...
  if (isLikelyHallucinated) {
    suggestion =
      "Use wit-get-work-items-by-query-wiql with returnQueryHandle=true, then pass the query handle to bulk operations instead of manual IDs.";
  }

  if (isLikelyHallucinated) {
    logger.warn(
      `🚨 Hallucination detected! Confidence: ${(confidence * 100).toFixed(0)}%. Reasons: ${reasons.join("; ")}`
    );
  }
 ...
 */
export function detectSuspiciousId(
  id: number,
  context?: { projectAge?: "new" | "established" }
): boolean {
...
export function validateIdPattern(ids: number[]): { valid: boolean; message: string } {
  ...
  if (invalidIds.length > 0) {
    return {
      valid: false,
      message: `Invalid IDs: ${invalidIds.join(", ")} (must be positive integers)`,
    };
  }
