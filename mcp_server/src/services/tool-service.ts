...
export async function executeTool(name: string, args: unknown): Promise<ToolExecutionResult> {
  ...
  if (isAIPoweredTool(name)) {
    if (!serverInstance) {
      throw new Error(
        `Tool '${name}' requires sampling support but no server instance is available. This tool uses AI-powered analysis and requires VS Code language model access.`
      );
    }
    ...
    if (!hasSampling) {
      throw new Error(
        `Tool '${name}' requires sampling support. This tool uses AI-powered analysis via VS Code's language model API. Please ensure you are using this MCP server within VS Code with language model access enabled, or use alternative non-AI tools for your task.`
      );
    }
  }
  ...
  if (name === "wit-ai-intelligence") {
    ...
    return await samplingService.analyzeWorkItem(
      args as Parameters<typeof samplingService.analyzeWorkItem>[0]
    );
  }
  ...
  if (name === "wit-ai-assignment") {
    ...
    const samplingService = new SamplingService(serverInstance);
    return await samplingService.analyzeAIAssignment(
      args as Parameters<typeof samplingService.analyzeAIAssignment>[0]
    );
  }
  ...
  if (name === "wit-ai-workload") {
    ...
    const samplingService = new SamplingService(serverInstance);
    return await samplingService.analyzePersonalWorkload(
      args as Parameters<typeof samplingService.analyzePersonalWorkload>[0]
    );
  }
  ...
  if (name === "wit-ai-sprint-planning") {
    ...
    const samplingService = new SamplingService(serverInstance);
    return await samplingService.analyzeSprintPlanning(
      args as Parameters<typeof samplingService.analyzeSprintPlanning>[0]
    );
  }
  ...
  if (name === "wit-ai-discover-tools") {
    ...
    const samplingService = new SamplingService(serverInstance);
    return await samplingService.discoverTools(
      args as Parameters<typeof samplingService.discoverTools>[0]
    );
  }
  ...
  if (name === "wit-get-context") {
    return await handleGetWorkItemContextPackage(
      args as Parameters<typeof handleGetWorkItemContextPackage>[0]
    );
  }

  // Batch context package (graph of work items)
  if (name === "wit-get-context-batch") {
    return await handleGetWorkItemsContextBatch(
      args as Parameters<typeof handleGetWorkItemsContextBatch>[0]
    );
  }

  // Get last substantive change for a work item
  if (name === "wit-get-last-change") {
    const { getLastSubstantiveChange } = await import(
      "./handlers/analysis/get-last-substantive-change.handler.js"
    );
    const result = await getLastSubstantiveChange(
      args as Parameters<typeof getLastSubstantiveChange>[0]
    );
    return {
      success: true,
  ...
  throw new Error(
    `Tool '${name}' is not properly registered. PowerShell script execution has been deprecated - all tools must be handled via TypeScript handlers.`
  );
}
