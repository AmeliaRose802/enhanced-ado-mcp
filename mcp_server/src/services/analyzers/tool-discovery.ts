    ...
    try {
      ...
      if (!this.samplingClient.hasSamplingSupport()) {
        return {
          success: false,
          errors: [
            "AI sampling not available - tool discovery requires VS Code language model access",
          ],
      ...
      switch (category) {
        ...
        case "ai-powered":
          return (
            ...
            name.includes("bulk-enhance") ||
            name.includes("bulk-assign-story") ||
  ...
  private fallbackRecommendations(intent: string): ToolRecommendation[] {
    ...
    if (recommendations.length === 0) {
      recommendations.push({
        ...
        confidence: 30,
        reasoning:
          "No clear match found - suggesting configuration tool to explore available options",
        requiredParameters: [],
