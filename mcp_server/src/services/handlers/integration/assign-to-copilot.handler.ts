...
export async function handleAssignToCopilot(
  config: ToolConfig,
  args: unknown
  ...
  try {
    ...
    const response = buildSuccessResponse(result, { tool: "wit-assign-copilot" });
    response.warnings = result.warnings || [];
    return response;
  } catch (error) {
    logger.error("Failed to assign work item to Copilot", error);
    return buildErrorResponse(error as Error, { source: "assign-to-copilot" });
  }
}
