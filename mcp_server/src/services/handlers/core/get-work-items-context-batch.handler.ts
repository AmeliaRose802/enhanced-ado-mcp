...
export async function handleGetWorkItemsContextBatch(args: BatchContextArgs) {
  ...
  try {
    ...
    const allFields = [
      ...new Set([
        ...baseFields,
        ...(includeExtendedFields ? extendedFields : []),
        ...includeFields,
      ]),
    ];
    ...
    for (const wi of details.value) {
      ...
      if (wi.relations) {
        for (const rel of wi.relations) {
          ...
          if (relType === "System.LinkTypes.Hierarchy-Forward") {
            ...
            if (
              artifactUrl.includes("/pullrequest/") ||
              artifactUrl.includes("vstfs:///Git/PullRequestId")
            ) {
              linkedPRCount++;
            } else if (
              artifactUrl.includes("/commit/") ||
              artifactUrl.includes("vstfs:///Git/Commit")
            ) {
              linkedCommitCount++;
            }
          }
      ...
      if (includeTags && f["System.Tags"]) {
        node.tags = String(f["System.Tags"])
          .split(/;|,/)
          .map((t: string) => t.trim())
          .filter(Boolean);
      }
      ...
      if (includeRelations && wi.relations) {
        for (const rel of wi.relations) {
          ...
          if (relType === "System.LinkTypes.Hierarchy-Forward") {
            ...
            if (
              !insideSet.has(targetId) &&
              includeChildrenOutsideSet &&
              outsideRefs.size < maxOutsideReferences
            ) {
    ...
    if (outsideRefs.size) {
      try {
        ...
        logger.warn("Failed to expand outside references", e);
      }
    }
    ...
    if (includeStateCounts) {
      aggregates.stateCounts = nodes.reduce((acc: Record<string, number>, n) => {
        acc[n.state] = (acc[n.state] || 0) + 1;
        return acc;
      }, {});
      aggregates.typeCounts = nodes.reduce((acc: Record<string, number>, n) => {
    ...
    if (includeStoryPointAggregation) {
      ...
      aggregates.storyPoints = {
        ...
        count: points.length,
        average:
          points.length > 0 ? points.reduce((a: number, b: number) => a + b, 0) / points.length : 0,
    ...
    if (includeRiskScoring) {
      ...
      const riskItems = nodes.filter(
        (n) =>
          (n.priority === 1 || n.priority === 2) && !["Done", "Closed", "Removed"].includes(n.state)
      );
    ...
    if (includeAIAssignmentHeuristic) {
      ...
      const aiCandidates = nodes.filter(
        (n) =>
          ["Task", "Bug", "User Story", "Product Backlog Item"].includes(n.type) &&
          ["New", "Active", "To Do", "Proposed"].includes(n.state) &&
          (n.storyPoints == null || n.storyPoints <= 3)
      );
    ...
    const data =
      returnFormat === "graph" ? graph : { items: nodes, edges: dedupedEdges, aggregates };
