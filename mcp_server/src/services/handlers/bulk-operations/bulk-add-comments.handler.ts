...
export async function handleBulkAddComments(
  config: ToolConfig,
  args: unknown
  ...
  try {
    ...
    logger.warn(
      "⚠️ DEPRECATION: wit-bulk-add-comments accepts manual IDs which increases hallucination risk. Consider using wit-bulk-comment with a query handle from wit-query-wiql instead."
    );
    ...
    const { items, template, templateVariables, organization, project } =
