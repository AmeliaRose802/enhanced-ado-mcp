...
export async function handleBulkUpdateByQueryHandle(
  config: ToolConfig,
  args: unknown
  ...
  try {
    ...
    const { queryHandle, updates, itemSelector, dryRun, maxPreviewItems, organization, project } =
      parsed.data;
    ...
    logger.info(
      `Bulk update operation: ${selectedCount} of ${totalItems} work items selected (dry_run: ${dryRun})`
    );
    ...
    if (invalidItems.length > 0) {
      logger.warn(
        `${invalidItems.length} items may not accept all field updates due to missing context`
      );
    }
    ...
    const fullSummary = failureCount > 0 ? `${summary} (${failureCount} failed)` : summary;
    ...
    if (failureCount > 0) {
      return {
        ...
        errors: results
          .filter((r) => !r.success)
          .map((r) => `Work item ${r.workItemId}: ${r.error}`),
        warnings: [],
      };
    }

    return buildSuccessResponse(responseData, { source: "bulk-update-by-query-handle" });
  } catch (error) {
    logger.error("Bulk update by query handle error:", error);
    return buildErrorResponse(error as Error, { source: "bulk-update-by-query-handle" });
  }
}
