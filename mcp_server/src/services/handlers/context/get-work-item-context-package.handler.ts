...
export async function handleGetWorkItemContextPackage(args: ContextPackageArgs) {
  ...
  try {
    ...
    if (parent && parent.id) {
      try {
        const pResponse = await httpClient.get<ADOWorkItem>(
          `wit/workitems/${parent.id}?fields=System.Id,System.Title,System.WorkItemType,System.State`
        );
        parent = pResponse.data;
      } catch (e) {
        logger.warn(`Failed to expand parent ${parent.id}`, e);
      }
    }
    ...
    if (children.length && maxChildDepth > 0) {
      ...
      const cResponse = await httpClient.get<ADOApiResponse<ADOWorkItem[]>>(
        `wit/workitems?ids=${idsParam}&fields=System.Id,System.Title,System.WorkItemType,System.State`
      );
    ...
    if (related.length > 0) {
      try {
        const relatedIds = related
          .map((r) => parseInt(r.url.split("/").pop() || "0", 10))
          .filter((id) => id > 0);
        if (relatedIds.length > 0) {
          const relIdsParam = relatedIds.slice(0, maxRelatedItems).join(",");
          const relResponse = await httpClient.get<ADOApiResponse<ADOWorkItem[]>>(
            `wit/workitems?ids=${relIdsParam}&fields=System.Id,System.Title,System.WorkItemType,System.State`
          );
        ...
        logger.warn("Failed to expand related items", e);
      }
    }
    ...
    if (includeComments) {
      try {
        const commentsResponse = await httpClient.get<ADOCommentsResponse>(
        ...
        logger.warn(`Failed to load comments for ${workItemId}`, e);
      }
    }
    ...
    if (includeHistory) {
      try {
        const hResponse = await httpClient.get<ADORevisionsResponse>(
          `wit/workItems/${workItemId}/revisions?$top=${maxHistoryRevisions}`
        );
        ...
        history = (hRes.value || [])
          ...
          .map((r) => {
            ...
            return {
              rev: r.rev || 0,
              changedDate:
                typeof cleanedFields?.["System.ChangedDate"] === "string"
                  ? cleanedFields["System.ChangedDate"]
                  : "",
              changedBy:
                typeof cleanedFields?.["System.ChangedBy"] === "string"
                  ? cleanedFields["System.ChangedBy"]
                  : undefined,
              fields: cleanedFields,
            };
          });
      } catch (e) {
        logger.warn(`Failed to load history for ${workItemId}`, e);
      }
    }
    ...
    const result = {
      ...
      assignedTo:
        fieldsMap["System.AssignedTo"]?.displayName || fieldsMap["System.AssignedTo"]?.uniqueName,
      createdDate: fieldsMap["System.CreatedDate"],
      createdBy:
        fieldsMap["System.CreatedBy"]?.displayName || fieldsMap["System.CreatedBy"]?.uniqueName,
      changedDate: fieldsMap["System.ChangedDate"],
      changedBy:
        fieldsMap["System.ChangedBy"]?.displayName || fieldsMap["System.ChangedBy"]?.uniqueName,
      ...
      acceptanceCriteria: processHtmlField(
        fieldsMap["Microsoft.VSTS.Common.AcceptanceCriteria"] as string | undefined,
        includeHtmlFields,
        stripHtmlFormatting
      ),
      reproSteps: fieldsMap["Microsoft.VSTS.TCM.ReproSteps"]
        ? processHtmlField(
            fieldsMap["Microsoft.VSTS.TCM.ReproSteps"] as string,
            includeHtmlFields,
            stripHtmlFormatting
          )
        : undefined,
      description: includeHtml
        ...
        : processHtmlField(
            fieldsMap["System.Description"] as string | undefined,
            includeHtmlFields,
            stripHtmlFormatting
          ),
      tags:
        includeTags && fieldsMap["System.Tags"]
          ? String(fieldsMap["System.Tags"])
              .split(/;|,/)
              .map((t: string) => t.trim())
              .filter(Boolean)
          : [],
