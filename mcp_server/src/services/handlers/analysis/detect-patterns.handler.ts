...
export async function handleDetectPatterns(
  config: ToolConfig,
  args: unknown
  ...
  try {
    ...
    if (patterns.includes("unassigned_committed")) {
      ...
      for (const item of workItems) {
        if (committedStates.includes(item.state) && !item.assignedTo) {
          ...
          patternSummary["unassigned_committed"] =
            (patternSummary["unassigned_committed"] || 0) + 1;
        }
      }
    }
    ...
    if (patterns.includes("no_description")) {
      for (const item of workItems) {
        ...
        const descriptionText =
          typeof description === "string" ? description.replace(/<[^>]*>/g, "").trim() : "";
        if (descriptionText.length < 50 && item.type !== "Epic") {
          // Epics often have minimal descriptions
          matches.push({
            workItemId: item.id,
            title: item.title,
    ...
    return buildErrorResponse(error as Error, { source: "detect-patterns" });
  }
}
