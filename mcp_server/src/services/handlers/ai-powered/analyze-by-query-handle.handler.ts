...
export async function handleAnalyzeByQueryHandle(
  ...
  try {
    ...
    logger.info(
      `Analyzing ${workItemIds.length} work items via query handle (analysis types: ${analysisType.join(", ")})`
    );
    ...
    try {
      ...
      if (workItemIds.length > 0) {
        ...
        for (let i = 0; i < workItemIds.length; i += batchSize) {
          ...
          try {
            ...
            throw new Error(
              `Failed to fetch work items batch for analysis: ${error instanceof Error ? error.message : String(error)}`
            );
          }
        }
      ...
      return {
        ...
        metadata: { source: "analyze-by-query-handle" },
        errors: [
          `Failed to fetch work items for analysis: ${error instanceof Error ? error.message : String(error)}`,
        ],
        warnings: [],
      };
    }
...
function analyzeEffort(workItems: ADOWorkItem[]): EffortAnalysisResult {
  const withStoryPoints = workItems.filter(
    (wi) => wi.fields?.["Microsoft.VSTS.Scheduling.StoryPoints"]
  );
  const totalStoryPoints = withStoryPoints.reduce(
    (sum, wi) => sum + (wi.fields["Microsoft.VSTS.Scheduling.StoryPoints"] || 0),
    0
  );
  ...
  return {
    ...
    average_story_points:
      withStoryPoints.length > 0
        ? Math.round((totalStoryPoints / withStoryPoints.length) * 10) / 10
        : 0,
...
function analyzeVelocity(workItems: ADOWorkItem[]): VelocityAnalysisResult {
  ...
  completedItems.forEach((wi) => {
    ...
    if (changedDate) {
      ...
      completedByMonth[month].storyPoints +=
        wi.fields?.["Microsoft.VSTS.Scheduling.StoryPoints"] || 0;
    }
  });
  ...
  return {
    ...
    avg_monthly_items:
      Object.keys(completedByMonth).length > 0
        ? Math.round(completedItems.length / Object.keys(completedByMonth).length)
        : 0,
    avg_monthly_story_points:
      Object.keys(completedByMonth).length > 0
        ? Math.round(
            Object.values(completedByMonth).reduce((sum, month) => sum + month.storyPoints, 0) /
              Object.keys(completedByMonth).length
          )
        : 0,
  };
}
...
function analyzeAssignments(workItems: ADOWorkItem[]): AssignmentAnalysisResult {
  ...
  workItems.forEach((wi) => {
    ...
    const assignedTo =
      typeof assignedToField === "object" && assignedToField?.displayName
  ...
  return {
    ...
    assignment_coverage: Math.round(
      ((workItems.length - unassignedCount) / workItems.length) * 100
    ),
  };
}
...
function analyzeRisks(workItems: ADOWorkItem[]): RiskAnalysisResult {
  ...
  const unestimated = workItems.filter(
    (wi) => !wi.fields?.["Microsoft.VSTS.Scheduling.StoryPoints"]
  );
  if (unestimated.length > workItems.length * 0.2) {
    risks.push(
      `High unestimated work: ${unestimated.length}/${workItems.length} items (${Math.round((unestimated.length / workItems.length) * 100)}%) lack Story Points`
    );
    riskScore += 25;
  }
  ...
  if (unassignedHighPriority.length > 0) {
    risks.push(
      `Unassigned high-priority: ${unassignedHighPriority.length} high-priority items lack assignment`
    );
...
function analyzeCompletion(workItems: ADOWorkItem[]): CompletionAnalysisResult {
  ...
  return {
    ...
    health_indicator:
      completed.length > active.length + backlog.length
        ? "Healthy"
        : active.length > backlog.length
          ? "Making Progress"
          : "Planning Heavy",
  };
}
...
function analyzePriorities(workItems: ADOWorkItem[]): PriorityAnalysisResult {
  ...
  const highPriority = workItems.filter(
    (wi) => (wi.fields?.["Microsoft.VSTS.Common.Priority"] || 999) <= 2
  );
  const mediumPriority = workItems.filter((wi) => {
    const prio = wi.fields?.["Microsoft.VSTS.Common.Priority"] || 999;
    return prio >= 3 && prio <= 6;
  });
  const lowPriority = workItems.filter(
    (wi) => (wi.fields?.["Microsoft.VSTS.Common.Priority"] || 999) > 6
  );
