...
export async function handleGenerateODataQuery(
  config: ToolConfig,
  args: unknown,
        ...
        if (!cumulativeUsage) {
          ...
          if (inputTokens)
            cumulativeUsage.inputTokens =
              ((cumulativeUsage.inputTokens as number) || 0) + inputTokens;
          if (outputTokens)
            cumulativeUsage.outputTokens =
              ((cumulativeUsage.outputTokens as number) || 0) + outputTokens;
          if (totalTokens)
            cumulativeUsage.totalTokens =
              ((cumulativeUsage.totalTokens as number) || 0) + totalTokens;
        }
      }
    ...
    if (returnQueryHandle && isValid && currentQuery) {
      try {
        ...
        if (!fullQuery.includes("$top")) {
          fullQuery +=
            (fullQuery.includes("?") || fullQuery.includes("$") ? "&" : "?") + `$top=${maxResults}`;
        }
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to execute OData query: ${response.status} ${response.statusText} - ${errorText}`
          );
        }
