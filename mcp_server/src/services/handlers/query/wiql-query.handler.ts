...
export async function handleWiqlQuery(
  ...
  try {
    ...
    if (parsed.data.includeSubstantiveChange) {
      logger.debug(
        `Substantive change analysis enabled (history depth: ${parsed.data.substantiveChangeHistoryCount || 50})`
      );
    }
    ...
    if (parsed.data.fetchFullPackages) {
      ...
      logger.info(
        `Successfully fetched ${fullPackages?.length ?? 0} of ${result.workItems.length} context packages`
      );
    }
    ...
    if (parsed.data.returnQueryHandle) {
      ...
      for (const wi of result.workItems) {
        ...
        workItemContext.set(wi.id, {
          ...
          ...(wi.lastSubstantiveChangeDate && {
            lastSubstantiveChangeDate: wi.lastSubstantiveChangeDate,
          }),
      ...
      const analysisMetadata = {
        ...
        successCount: result.workItems.filter((wi) => wi.lastSubstantiveChangeDate !== undefined)
          .length,
        failureCount:
          result.workItems.length -
          result.workItems.filter((wi) => wi.lastSubstantiveChangeDate !== undefined).length,
      };
      ...
      return {
        ...
        data: {
          ...
          next_steps: [
            ...
            ...(fullPackages
              ? [
                  "Review the full_packages array for detailed context including descriptions, comments, relations, and history",
                ]
              : []),
