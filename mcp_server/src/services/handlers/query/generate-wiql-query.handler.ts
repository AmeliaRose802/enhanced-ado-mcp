  ...
  try {
    ...
    const result: ToolExecutionResult = {
      ...
      data: {
        ...
        summary: isValid
          ? `Successfully generated and validated WIQL query in ${iterations.length} iteration(s)${testResults ? ` (found ${testResults.resultCount} matching work items)` : ''}`
          : `Generated WIQL query but validation failed after ${maxIterations} iterations. Last error: ${lastError}`
      },
      metadata: {
      ...
      warnings: [
        ...(iterations.length > 1 ? [`Required ${iterations.length} iterations to generate valid query`] : []),
...
async function generateQueryWithAI(
  ...
  if (iterationPath) {
    variables.ITERATION_PATH = `\n- Default Iteration Path: ${iterationPath}`;
  } else {
    variables.ITERATION_PATH = '';
  }
