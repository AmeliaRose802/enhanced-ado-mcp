...
export const queryTools: ToolConfig[] = [
  {
    name: "query-wiql",
    description: "🔐 UNIFIED WIQL TOOL: Execute WIQL queries (direct or AI-generated from natural language) and get both work item details AND a query handle for safe bulk operations. TWO MODES: (1) handleOnly=true: returns ONLY query handle + count (efficient, use inspect-handle to get data later), (2) handleOnly=false (default): returns handle + full work item data immediately. Supports AI-powered query generation with automatic validation. ⚠️ CRITICAL: Do not reference work item IDs directly - use the returned query_handle to prevent ID hallucination. When using handleOnly=true, DO NOT add includeFields, includeSubstantiveChange, or maxResults parameters - they are ignored in that mode.",
    script: "",
    ...
    inputSchema: {
      ...
      properties: {
        ...
        handleOnly: { type: "boolean", description: "⚡ EFFICIENCY MODE: When true (with returnQueryHandle=true), returns ONLY the query handle and count without fetching full work item data. Use this for bulk operations where you don't need to preview/analyze items first - saves API calls and response payload. To retrieve items later, use inspect-handle with the handle. Default: false. ⚠️ When true, includeFields, includeSubstantiveChange, and fetchFullPackages parameters are IGNORED." },
        maxResults: { type: "number", description: "Maximum number of results to return per page (default 200, max 10000). ⚠️ Results are truncated at this limit - use pagination for more. Has NO EFFECT when handleOnly=true." },
        ...
        // Data Enrichment
        includeFields: { type: "array", items: { type: "string" }, description: "⚠️ ONLY USED WHEN handleOnly=false: Additional fields to include in work item data (e.g., 'System.Description', 'Microsoft.VSTS.Common.Priority'). Has NO EFFECT when handleOnly=true - in that mode, use inspect-handle or get-context-bulk to retrieve specific fields." },
        includeSubstantiveChange: { type: "boolean", description: "⚠️ ONLY USED WHEN handleOnly=false: Include computed fields lastSubstantiveChangeDate and daysInactive for each work item - automatically filters out automated changes. Essential for backlog hygiene workflows. Has NO EFFECT when handleOnly=true - substantive change data must be calculated separately." },
        fetchFullPackages: { type: "boolean", description: "⚠️ ONLY USED WHEN handleOnly=false: Fetch full context packages for each work item including description, comments, history, relations, children, and parent. ⚠️ WARNING: Increases API calls significantly (1 call per work item + relations/comments). Use for deep analysis of small result sets (<50 items). Automatically includes extended fields, relations, comments, and history." },
