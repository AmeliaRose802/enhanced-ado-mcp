...
const mockQueryWorkItemsByWiql = queryWorkItemsByWiql as jest.MockedFunction<
  typeof queryWorkItemsByWiql
>;
...
describe("Generate WIQL Query Handler - returnQueryHandle parameter", () => {
    ...
    it("should return only the generated query without creating a handle", async () => {
      ...
      mockSamplingClient.createMessage.mockResolvedValueOnce({
        content: [
          {
            type: "text",
            text: "SELECT [System.Id], [System.Title] FROM WorkItems WHERE [System.State] = 'Active'",
          },
        ],
        usage: { inputTokens: 100, outputTokens: 50 },
      });
      mockSamplingClient.extractResponseText.mockReturnValueOnce(
      ...
      mockQueryWorkItemsByWiql.mockResolvedValueOnce({
        workItems: [
          { id: 123, title: "Test", type: "Task", state: "Active", url: "http://test.com/123" },
        ],
        totalCount: 1,
        count: 1,
  ...
  describe("when returnQueryHandle is true", () => {
    ...
    it("should handle query with no results gracefully", async () => {
      ...
      mockSamplingClient.createMessage.mockResolvedValueOnce({
        content: [
          {
            type: "text",
            text: "SELECT [System.Id] FROM WorkItems WHERE [System.State] = 'NonExistent'",
          },
        ],
        usage: { inputTokens: 100, outputTokens: 50 },
      });
      mockSamplingClient.extractResponseText.mockReturnValueOnce(
        "SELECT [System.Id] FROM WorkItems WHERE [System.State] = 'NonExistent'"
      );

      // Mock query validation (first call - returns something to pass validation)
      mockQueryWorkItemsByWiql.mockResolvedValueOnce({
        workItems: [
          { id: 999, title: "Test", type: "Task", state: "Active", url: "http://test.com/999" },
        ],
        totalCount: 1,
        count: 1,
        query: "SELECT [System.Id] FROM WorkItems",
    ...
    it("should include custom fields when specified", async () => {
      ...
      expect(result.data.work_items[0].additionalFields["Microsoft.VSTS.Common.Severity"]).toBe(
        "High"
      );
