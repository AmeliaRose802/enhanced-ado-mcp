  ...
  describe("with criteria selector", () => {
    ...
    it("should select items by daysInactive criteria", async () => {
      ...
      const itemContext = new Map([
        [
          12345,
          { id: 12345, title: "Item 1", state: "Active", type: "Task", daysInactive: 30, index: 0 },
        ],
        [
          67890,
          { id: 67890, title: "Item 2", state: "New", type: "Bug", daysInactive: 95, index: 1 },
        ],
        [
          11111,
          {
    ...
    it("should combine multiple criteria with AND logic", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data.selection_analysis.selected_items_count).toBe(2);
      expect(result.data.selection_analysis.criteria_used).toEqual([
        "states",
        "tags",
        "daysInactiveMin",
      ]);
