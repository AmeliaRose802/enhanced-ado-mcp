...
describe("Detect Patterns - Format Options", () => {
  ...
  it("should detect placeholder patterns", async () => {
    ...
    const placeholderMatch = allMatches.find(
      (m: any) => m.workItemId === 101 && m.patterns.includes("placeholder_titles")
    );
  ...
  it("should detect duplicates", async () => {
    ...
    const duplicateMatches = allMatches.filter(
      (m: any) => m.patterns.includes("duplicates") && [102, 103].includes(m.workItemId)
    );
