...
class MockQueryHandleService {
  ...
  selectByCriteria(handle: string, criteria: any) {
    ...
    if (criteria.daysInactiveMin !== undefined) {
      filtered = filtered.filter((item: any) => 
        (item.daysInactive ?? 0) >= criteria.daysInactiveMin
      );
    }
    
    if (criteria.daysInactiveMax !== undefined) {
      filtered = filtered.filter((item: any) => 
        (item.daysInactive ?? 0) <= criteria.daysInactiveMax
      );
    }
...
export async function runQueryBenchmarks(): Promise<void> {
  ...
  bench.add('WIQL Complex Query (200 items)', () => {
    const wiql = 'SELECT [System.Id] FROM WorkItems WHERE [System.State] = \'Active\' AND [System.WorkItemType] = \'Task\' AND [System.Tags] CONTAINS \'important\'';
  ...
  const taskMapping = {
    ...
    'Query Handle Select Criteria (1000 items)': PERFORMANCE_THRESHOLDS.QUERY_HANDLE_SELECT_CRITERIA
  };
