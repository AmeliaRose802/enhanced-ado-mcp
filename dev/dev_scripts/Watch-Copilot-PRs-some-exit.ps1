...
for ($i = 1; $i -le $MaxPolls; $i++) {
    ...
    # Check if ANY PR is complete (exit early strategy)
    if ($script:completedPRs.Count -gt 0) {
        $remainingPRs = $PRNumbers | Where-Object { $_ -notin $script:completedPRs }
        
        Write-Host ""
        Write-Host ("=" * 80) -ForegroundColor Green
        Write-Host "✅ PR(S) READY FOR MERGE!" -ForegroundColor Green -BackgroundColor DarkGreen
        Write-Host ("=" * 80) -ForegroundColor Green
        Write-Host ""
        ...
        Write-Host "Remaining PRs: $($remainingPRs -join ', ')" -ForegroundColor Yellow
        Write-Host ""
        Write-Host "✅ Copilot agent(s) have finished their work" -ForegroundColor Green
        Write-Host "✅ CI checks have passed" -ForegroundColor Green
        Write-Host "✅ PR(s) mergeable" -ForegroundColor Green
        Write-Host ""
        Write-Host "🔍 Next Steps:" -ForegroundColor Yellow
        Write-Host "1. Merge completed PR(s): $($script:completedPRs -join ', ')" -ForegroundColor White
        Write-Host "2. Run integration tests: npm test && npm run build" -ForegroundColor White
        Write-Host "3. Schedule dependent tasks immediately" -ForegroundColor White
        Write-Host "4. Resume monitoring remaining PRs: $($remainingPRs -join ', ')" -ForegroundColor White
        Write-Host ""
        
        Invoke-CompletionSound -Success $true
        
        # Exit with code 0 and output completed PR numbers for orchestrator to parse
        Write-Output "COMPLETED_PRS=$($script:completedPRs -join ',')"
        Write-Output "REMAINING_PRS=$($remainingPRs -join ',')"
        exit 0
    }
